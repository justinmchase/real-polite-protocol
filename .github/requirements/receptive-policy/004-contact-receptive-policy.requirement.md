...
A user MAY create a receptive policy with `mode: "contact"` that accepts
invitations only from senders whose composite `(sender_domain, domain_id)` pair
appears in the policy's `contacts` list.
...
6. Contact policies do not expire unless a `receptive_until` is also set.
