...
3. When the submit endpoint receives an invitation with `receipt_id`: a. Look up
   the receipt; if not found or not active → reject `E_RECEIPT_NOT_ACTIVE`. b.
   Verify a `mode: "receipt"` policy for this `receipt_id` exists on
   `receipt.oid` → if not, reject `E_RECEPTIVE_POLICY_NOT_FOUND`. c. Create the
   invitation as `pending` on `receipt.oid`.
