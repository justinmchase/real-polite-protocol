...
- After bearer token validation, the server resolves the Account for the
  request and makes it available to all tool handlers.
...
- A tool that creates or mutates resources MUST associate the new record with
  the calling Account.
- Domain management tools (Section 10B.7) require the domain administrator
  role; account ownership alone is not sufficient.
