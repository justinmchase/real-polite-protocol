  ...
  fn: async (t) => {
    await withAuthTestContext(async ({ issueToken }) => {
      await withStartedServer(async ({ callTool, baseUrl }) => {
        ...
        await t.step(
          "domain_id appears in immutable_fields and verified_fields",
          async () => {
        ...
        await t.step(
          "set_admin_verified_metadata rejects attempts to overwrite domain_id",
          async () => {
        ...
        await t.step(
          "domain_id is unchanged after rejected overwrite attempt",
          async () => {
            const metadata = await callTool(
              adminToken,
              "get_user_verified_metadata",
        ...
        await t.step(
          "remove_admin_verified_metadata rejects attempts to delete domain_id",
          async () => {
            const { status, body } = await callTool(
              adminToken,
        ...
        await t.step(
          "domain_id is still present after rejected remove attempt",
          async () => {
            const metadata = await callTool(
              adminToken,
              "get_user_verified_metadata",
