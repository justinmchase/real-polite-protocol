...
Deno.test("req:account-005 - Domain role authorization derives from token roles claim", async (t) => {
  await withAuthTestContext(async ({ issueToken }) => {
    await withStartedServer(async () => {
      await t.step("domain.admin in token roles grants domain-level permissions", async () => {
      ...
      await t.step("absence of domain.admin keeps account in listener-only permissions", async () => {
        const token = await issueToken({
      ...
      await t.step("mutable request arguments do not override token-derived role evaluation", async () => {
        const token = await issueToken({
