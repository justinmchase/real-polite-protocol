...
Deno.test({
  name:
    "req:account-010 - Sender domain_id is always included as admin-verified claim on outgoing invitations",
  fn: async (t) => {
    await withAuthTestContext(async ({ issueToken }) => {
      await withStartedServer(async ({ callTool, baseUrl }) => {
        ...
        const metadataResult = await callTool(
          adminToken,
        ...
        await t.step(
          "invitation without explicit claim inputs still includes domain_id as admin-verified claim",
          async () => {
            ...
            const { status: reviewStatus, result: reviewResult } =
              await callTool<{
                claims?: {
            ...
            assertExists(
              domainId,
              "domain_id must be present in immutable claims",
            );
            assertMatch(
              domainId,
        ...
        await t.step(
          "domain_id is present alongside other caller-requested admin claims",
          async () => {
            const { status, result } = await callTool<
              { invitation_id?: string }
            >(
        ...
        await t.step(
          "domain_id on invitation is the sender's, not a receiver value",
          async () => {
