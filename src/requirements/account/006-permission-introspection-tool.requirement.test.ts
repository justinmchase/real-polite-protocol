...
Deno.test("req:account-006 - MCP exposes get_permissions for current account", async (t) => {
  await withAuthTestContext(async ({ issueToken }) => {
    await withStartedServer(async () => {
      await t.step("returns domain permissions when roles includes domain.admin", async () => {
      ...
      await t.step("returns listener permissions without domain.admin role", async () => {
        const token = await issueToken({
          oid: "oid-listener",
          roles: ["rpp.user"],
          scope: requiredScopes.join(" "),
