...
Deno.test({
  name: "req:mcp-auth-006 - OAuth discovery metadata is published for MCP authentication",
  fn: async (t) => {
    await withStartedServer(async () => {
      await t.step("serves protected resource metadata", async () => {
        ...
        assertEquals(body.authorization_servers.includes("http://localhost:8000"), true);
      ...
      await t.step("serves authorization server metadata", async () => {
        ...
        assertEquals(body.authorization_endpoint, "http://localhost:8000/authorize");
        assertEquals(body.token_endpoint, "http://localhost:8000/token");
        assertEquals(body.resource, "http://localhost:8000/mcp");
        assertEquals(body.response_types_supported.includes("code"), true);
        assertEquals(body.code_challenge_methods_supported.includes("S256"), true);
      });
    });
  },
});
