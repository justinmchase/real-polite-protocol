    ...
    await withAuthTestContext(async ({ issueToken }) => {
      await withStartedServer(async ({ kvPath }) => {
        ...
        try {
          ...
          await t.step("list_invitations returns all invitations for the user", async () => {
            const { status, result } = await callTool<{ invitations: Array<{ invitation_id: string }> }>(token, "list_invitations", {});
          ...
          await t.step("list_invitations can filter by status", async () => {
            const { status, result } = await callTool<{ invitations: Array<{ status: string }> }>(token, "list_invitations", {
              status: "pending",
            });
          ...
          await t.step("list_invitations can filter by sender domain", async () => {
            const { status, result } = await callTool<{ invitations: Array<{ sender_domain: string }> }>(token, "list_invitations", {
              sender_domain: "sender1.example",
            });
            ...
            assertExists(result);
            assertEquals(result.invitations.length, 1);
            assertEquals(result.invitations[0].sender_domain, "sender1.example");
          });

          await t.step("list_invitations respects page_size limit", async () => {
            const { status, result } = await callTool<{ invitations: Array<unknown>; page_size?: number }>(token, "list_invitations", {
              page_size: 1,
            });
