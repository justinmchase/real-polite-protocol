...
For manual end-to-end testing you often need a second identity besides the
Azure AD user backing your VS Code MCP session. The repo ships a dev-only
auth seam plus a small CLI for acting on the running server as a named
persona.
...
first use). Hand-edit `roles` to add `"domain.admin"` when exercising
admin-only tools. The signing keypair lives in `.dev/keys/` and is generated
on first use. Both directories are gitignored.

Override defaults with env vars: `RPP_SERVER` (default
`http://localhost:8000`), `RPP_DEV_ISSUER` (default `urn:rpp:dev`),
`RPP_DEV_PUBLIC_KEY_PATH`.
...
The following tools are available to authenticated users via the `/mcp` endpoint.
